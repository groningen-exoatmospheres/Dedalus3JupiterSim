...
lat2 = np.pi/6
en2 = np.exp(-4 /(lat2-lat1)**2)
jet2 = (lat1<=lat) * (lat<=lat2)
u_jet2 = -umax / en2 * np.exp(1 / (lat[jet2] - lat1) / (lat[jet2] - lat2))
...
# Analysis
snapshots = solver.evaluator.add_file_handler('jupiterGalewskyMid', sim_dt=1*hour, max_writes=10)
